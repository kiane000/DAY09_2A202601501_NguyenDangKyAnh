...
| MSSV            | 2A202601501 |
| Khóa/Lớp        | K3         |
| Vai trò chính   | Triển khai LLM client, 6 agent LLM và orchestration/coordinator; tích hợp & so sánh provider (Ollama local / OpenAI); debug lỗi confidence. Nhóm 2 người, chia module với Nguyễn Việt Hùng (data layer, policy engine, verification tooling — xem `individual_01275_NguyenVietHung.md`).   |
...
| ------------------ | ------------------ | -------------- | ----------------- | ------------------------------------- |
| LLM client (gọi model, retry theo schema) | `src/llm_client.py` | prompt + Pydantic model (schema do Hùng định nghĩa ở `src/schemas.py`) | JSON đã validate, tự retry khi sai schema | Hoàn thành, hỗ trợ 2 provider (Ollama local / OpenAI) |
| 6 agent + orchestration | `src/agents/*.py`, `src/coordinator.py`, `src/main.py` | `input/EC_xxx.json` + `OrderFacts` (từ `data_store.py`, do Hùng xây) + `PolicyDecision` (từ `policy_engine.py`, do Hùng xây) | `output/EC_xxx.json` theo đúng schema README §6 | Hoàn thành, chạy thật 50/50 case |
...
Phần `data_store.py` (data layer), `policy_engine.py` + `tests/test_policy_engine.py` (policy engine tất định), `schemas.py`, và `verifier_agent.py`/`scripts/validate_output.py` (verification & tooling) do Nguyễn Việt Hùng phụ trách — xem `individual_01275_NguyenVietHung.md`.
...
Phần tôi phụ trách: xây 6 agent LLM (`Order & Seller`, `Payment`, `Delivery`, `Policy`, `Verifier`, `Coordinator`) sao cho mỗi agent dùng LLM thật (≤10B theo đề bài) để phân tích, có handoff và kiểm chứng chéo với nhau — nhưng đầu ra cuối cùng (`primary_issue`, số tiền hoàn, evidence ID) vẫn phải đúng 100% vì "case bị hard gate nhận 0 điểm", nên không được để LLM tự quyết định các trường này.

### Cách triển khai

`Coordinator` gọi tuần tự 6 agent; mỗi agent nhận facts đã được `policy_engine.py` (module của Nguyễn Việt Hùng — hàm tất định áp đúng bảng ưu tiên 6 rule: canceled/unavailable/late_delivery_seller/late_delivery_logistics/valid_split_payment/unsupported_late_claim, là nguồn sự thật duy nhất cho `primary_issue`, số tiền, root cause, action) tính sẵn, không phải raw data cần tự suy luận số học — agent chỉ làm nhiệm vụ diễn giải/phân loại độc lập/kiểm chứng rồi handoff cho agent tiếp theo qua `src/llm_client.py` (module tôi xây, gọi model + validate JSON theo Pydantic schema). `Coordinator` ghép `CaseOutput` cuối cùng luôn từ kết quả `policy_engine`, không bao giờ từ output tự do của LLM — LLM chỉ ảnh hưởng đến `confidence` (qua cross-check với Policy Agent) và nội dung `logging/trace.jsonl`.
